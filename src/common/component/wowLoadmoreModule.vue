<template>
  <div class="outer" id="loadMoreMoudle" :style="{height:height+'px', width:width+'px'}">
...
export default {
  ...
  mounted () {
    var that = this
    var t = null
    document.getElementById('loadMoreMoudle').addEventListener('scroll', function (e) {
      if (t === true) return
      t = true
      setTimeout(() => {
        if (document.getElementById('loadMoreMoudle').scrollHeight - e.target.scrollTop <= that.height) {
          that.$emit('loadMore')
        }
        t = false
      }, 500)
